...
func TestInspectUsesNormalizedBoundedScreen(t *testing.T) {
	...
	got, err := detection.Inspect(t.Context(), registry.HarnessClaude, screen, detection.Options{ConfigDir: dir})
	if err != nil {
		t.Fatal(err)
	}
...
`
	...
	got, err := detection.Inspect(t.Context(), registry.HarnessCodex, "approval", detection.Options{ManifestPath: path, IncludeScreen: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := detection.Inspect(t.Context(), registry.HarnessClaude, "approval", detection.Options{ManifestPath: path}); err == nil {
...
func TestInspectInputFailures(t *testing.T) {
	t.Parallel()
	_, err := detection.Inspect(t.Context(), registry.HarnessCodex, strings.Repeat("x", detection.MaxScreenBytes+1), detection.Options{})
	if !errors.Is(err, detection.ErrScreenTooLarge) {
		t.Fatalf("oversize error = %v", err)
	}
	_, err = detection.Inspect(t.Context(), registry.HarnessCodex, "", detection.Options{ManifestPath: "rules.toml", ConfigDir: "rules"})
	if !errors.Is(err, detection.ErrManifestOptions) {
		t.Fatalf("conflicting options error = %v", err)
	}
	ctx, cancel := context.WithCancel(t.Context())
	cancel()
	_, err = detection.Inspect(ctx, registry.HarnessCodex, "", detection.Options{})
